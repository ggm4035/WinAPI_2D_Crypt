...
void CLongSword::Render_Text(HDC _hDC)
{
	...
	TextOut(_hDC, int(m_tInfo.fX + fScrollX), int(m_tInfo.fY + fScrollY - 60.f), L"longsword", 10);
...
void CLongSword::Initialize()
{
	...
	m_tEffect.iFrameEnd = 3;
	m_tEffect.dwSpeed = 100;
	m_tEffect.iMotion = 0;
	m_tEffect.iMotionEnd = 3;

	m_tState.iAttack = 1;
}

int CLongSword::Update()
{
	if (m_bDead)
		return OBJ_DEAD;
...
void CLongSword::Render(HDC _hDC)
{
	float fScrollX = SCROLL_MGR->Get_ScrollX();
	float fScrollY = SCROLL_MGR->Get_ScrollY();
...
void CLongSword::Release()
{
}

void CLongSword::Render_Effect(HDC _hDC)
{
	if (!m_bEffect)
		return;
	...
	case LEFT:
		...
		hEffectDC = BITMAP_MGR->Find_Img(L"swipe_longsword_gold_Left");
		break;

	case RIGHT:
		if (m_tEffect.iFrameStart == 3)
			m_bEffect = false;

		hEffectDC = BITMAP_MGR->Find_Img(L"swipe_longsword_gold");
		break;
	...
	case UP:
		...
		hEffectDC = BITMAP_MGR->Find_Img(L"swipe_longsword_gold_Up");
		break;

	case DOWN:
		if (m_tEffect.iMotion == 3)
			m_bEffect = false;

		hEffectDC = BITMAP_MGR->Find_Img(L"swipe_longsword_gold_Down");
		break;
...
void CLongSword::Attack(const DIRECTION & _eDir)
{
	...
	case LEFT:
		...
		m_tTarget.fCX = TILECX * 2;
		m_tTarget.fCY = TILECY;

		m_tEffect.iFrameStart = 3;
		m_tEffect.iMotion = 0;
		m_eOption = RTOL;
...
bool CLongSword::IsRange(const DIRECTION & _eDir)
{
	CObj* pObj = OBJ_MGR->Get_Player();
	...
	case LEFT:
		if (OBJ_MGR->Find_Obj(OBJ_MONSTER, pObj->Get_Info().fX - TILECX, pObj->Get_Info().fY) || 
			OBJ_MGR->Find_Obj(OBJ_MONSTER, pObj->Get_Info().fX - TILECX * 2, pObj->Get_Info().fY))
			return true;
		break;
	...
	case UP:
		if (OBJ_MGR->Find_Obj(OBJ_MONSTER, pObj->Get_Info().fX, pObj->Get_Info().fY - TILECY) ||
			OBJ_MGR->Find_Obj(OBJ_MONSTER, pObj->Get_Info().fX, pObj->Get_Info().fY - TILECY * 2))
			return true;
		break;
